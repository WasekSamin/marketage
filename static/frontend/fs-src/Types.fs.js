...
export class MessageTypeErrorOccured extends Record {
    constructor(error) {
        super();
        this.error = error;
    }
...
export function MessageTypeErrorOccured$reflection() {
    return record_type("App.AppTypes.MessageTypeErrorOccured", [], MessageTypeErrorOccured, () => [["error", tuple_type(enum_type("App.AppTypes.ErrorTypes", int32_type, [["MessageParsingError", 1], ["TextMessageInvalid", 2], ["InvalidMessageReadId", 3], ["InvalidUserPk", 4], ["InvalidRandomId", 5], ["FileMessageInvalid", 6], ["FileDoesNotExist", 7]]), string_type)]]);
}

export function MessageTypeErrorOccured_get_Decoder() {
    return (path_2) => ((v) => object((get$) => (new MessageTypeErrorOccured(get$.Required.Field("error", uncurry(2, tuple2((path, value_1) => andThen((value, arg10$0040, arg20$0040) => succeed(value, arg10$0040, arg20$0040), uncurry(2, int), path, value_1), (path_1, value_2) => string(path_1, value_2)))))), path_2, v));
}
